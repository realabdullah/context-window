...
## API Project (cw-api.abdspace.xyz)
...
| `GITHUB_CALLBACK_URL` | **Yes** | `https://cw-api.abdspace.xyz/auth/github/callback` |
| `NEXTJS_APP_URL` or `VITE_APP_URL` | **Yes** (for CORS) | Your frontend URL, e.g. `https://cw.abdspace.xyz` |
| `ALLOWED_ORIGINS` | Optional | Comma-separated origins to add (e.g. `https://cw.abdspace.xyz`) |

**CORS:** The API allows origins from `NEXTJS_APP_URL` and `VITE_APP_URL`. Set at least one to your production frontend URL. You can also use `ALLOWED_ORIGINS=https://cw.abdspace.xyz`.

### GitHub OAuth App Configuration

In [GitHub OAuth App settings](https://github.com/settings/developers):

1. **Authorization callback URL:** `https://cw-api.abdspace.xyz/auth/github/callback`
2. **Homepage URL:** `https://cw.abdspace.xyz` (or your frontend URL)
...
## Frontend Project (cw.abdspace.xyz)
...
| Variable | Required | Description |
|----------|----------|-------------|
| `VITE_API_URL` (Vite) or `NEXT_PUBLIC_API_URL` (Next.js) | **Yes** | `https://cw-api.abdspace.xyz` |
...
1. Ensure `NEXTJS_APP_URL` or `VITE_APP_URL` is set to `https://cw.abdspace.xyz` in the **API** project.
2. Or set `ALLOWED_ORIGINS=https://cw.abdspace.xyz` in the API project.
...
3. **Wrong `GITHUB_CALLBACK_URL`** – Must be `https://cw-api.abdspace.xyz/auth/github/callback` (your API domain).
