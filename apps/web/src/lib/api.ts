 ...
 * All LLM keys stay on the backend; client only talks to our API.
...
/** User as returned by GET /auth/me (session hydration). */
...
export async function updateTrace(id: string, payload: { title: string }): Promise<Trace> {
  const res = await fetch(`${getBaseUrl()}/traces/${id}`, {
    method: 'PATCH',
    ...defaultFetchOptions,
    headers: headers(),
